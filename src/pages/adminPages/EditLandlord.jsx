...
const EditLandlord = () => {
  ...
  // Store original images to track changes
    ...
    const loadLandlordData = async () => {
      try {
        ...
        if (
          ...
          const landlord = response.data.data.find(
            (l) => l.landlord_slug === landlordSlug
          );
          ...
          if (landlord) {
            setFormData({
              ...
              business_logo: landlord.business_logo || null,
              selfie_picture: landlord.selfie_picture || null,
              ghana_card_front: landlord.ghana_card_front || null,
              ghana_card_back: landlord.ghana_card_back || null,
              is_active: landlord.is_active || false,
              is_verified: landlord.is_verified || false,
            });

            // Set preview images for existing images
            if (landlord.business_logo) setLogoPreview(landlord.business_logo);
            if (landlord.selfie_picture)
              setSelfiePreview(landlord.selfie_picture);
            if (landlord.ghana_card_front)
              setGhanaCardFrontPreview(landlord.ghana_card_front);
            if (landlord.ghana_card_back)
              setGhanaCardBackPreview(landlord.ghana_card_back);
          ...
          toast.error(response?.data?.reason || "Failed to load landlord data");
          navigate("/admin/landlords");
        }
      } catch (error) {
  ...
  const removeVerificationImage = (field, setPreview) => {
    if (setPreview === setSelfiePreview && selfiePreview) {
      ...
      setPreview === setGhanaCardFrontPreview &&
  ...
  const steps = [
    ...
    { id: 4, title: "Review & Update", icon: CheckCircle2 },
  ];
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 1:
        ...
        if (!formData.phone_number.trim()) {
          ...
          newErrors.phone_number =
            "Please enter a valid 9-digit Ghana mobile number";
        }
      ...
      case 2:
        ...
        // For edit mode, check if we have original images OR new uploads
        // Images can be URLs (original) or base64 strings (new uploads)
        if (!formData.selfie_picture && !originalImages.selfie_picture) {
          newErrors.selfie_picture =
            "Selfie picture is required for verification";
        }

        if (!formData.ghana_card_front && !originalImages.ghana_card_front) {
          newErrors.ghana_card_front = "Ghana Card front image is required";
        }

        if (!formData.ghana_card_back && !originalImages.ghana_card_back) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      // Helper function to determine if image should be sent as base64 or URL
      const getImageValue = (fieldName) => {
        const currentValue = formData[fieldName];
        const originalValue = originalImages[fieldName];
        
        // If no image, send null
        if (!currentValue) return null;
        
        // If current value is same as original (URL), send URL
        if (currentValue === originalValue) return currentValue;
        
        // If changed, it's a new base64 image
        return currentValue;
      };
      ...
      const payload = {
        ...
        business_logo: getImageValue('business_logo'),
        selfie_picture: getImageValue('selfie_picture'),
        ghana_card_front: getImageValue('ghana_card_front'),
        ghana_card_back: getImageValue('ghana_card_back'),
        is_active: formData.is_active,
        is_verified: formData.is_verified,
        landlord_slug: landlordSlug,
      };

      const response = await generalRequests.updateLandlordDetails(
        landlordSlug,
        payload
      );
      ...
      if (!response?.data?.in_error) {
        toast.success(
          response?.data?.reason || "Landlord updated successfully!"
        );
        navigate("/admin/landlords");
      } else {
        toast.error(response?.data?.reason || "Failed to update landlord");
      }
    } catch (error) {
          ...
          >
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 rounded-2xl p-8 border border-blue-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div
                  className="space-y-2"
                  ref={(el) => (errorFieldRefs.current.full_name = el)}
                >
                ...
                <div
                  className="space-y-2"
                  ref={(el) => (errorFieldRefs.current.phone_number = el)}
                >
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <Phone size={14} />
                    Phone Number *
                ...
                <div
                  className="space-y-2"
                  ref={(el) => (errorFieldRefs.current.ghana_card_number = el)}
                >
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <Shield size={14} />
                    Ghana Card Number *
                ...
                >
                  ...
                  >
                    ...
                    >
                      <span
                        className={
                          formData.location ? "text-gray-900" : "text-gray-500"
                        }
                      >
          ...
          >
            <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-2xl p-8 border border-orange-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div
                  className="space-y-2"
                  ref={(el) => (errorFieldRefs.current.business_name = el)}
                >
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <Building size={14} />
                    Business Name *
                ...
                <div
                  className="space-y-2"
                  ref={(el) => (errorFieldRefs.current.business_type = el)}
                >
                  <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                    <Settings size={14} />
                    Business Type *
                    ...
                    >
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="flex items-center gap-2 text-sm text-gray-500">
                          ...
                          <span>
                            Recommended: Square format, 200x200px or larger
                          </span>
                        </div>
                      </div>
                ...
                <div className="space-y-6 md:col-span-2">
                  <div className="bg-gradient-to-r from-purple-50 to-purple-100 border border-purple-200 rounded-xl p-6">
                    ...
                    <p className="text-sm text-gray-700 mb-6">
                      Upload the required verification images to complete your
                      account setup.
                    </p>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div
                        className="space-y-2"
                        ref={(el) =>
                          (errorFieldRefs.current.selfie_picture = el)
                        }
                      >
                        <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                          <User size={14} />
                      ...
                      <div
                        className="space-y-2"
                        ref={(el) =>
                          (errorFieldRefs.current.ghana_card_front = el)
                        }
                      >
                        <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                          <Shield size={14} />
                        ...
                        {!ghanaCardFrontPreview ? (
                          <div
                            ...
                            onClick={() =>
                              document
                                .getElementById("ghana-card-front-upload")
                                .click()
                            }
                          >
                            <div className="flex flex-col items-center gap-3">
                            ...
                            <input
                              ...
                              accept="image/*"
                              onChange={(e) =>
                                handleGhanaCardFrontUpload(e.target.files[0])
                              }
                              className="hidden"
                            />
                          </div>
                      ...
                      <div
                        className="space-y-2"
                        ref={(el) =>
                          (errorFieldRefs.current.ghana_card_back = el)
                        }
                      >
                        <label className="text-sm font-medium text-gray-700 flex items-center gap-2">
                          <Shield size={14} />
                        ...
                        {!ghanaCardBackPreview ? (
                          <div
                            ...
                            onClick={() =>
                              document
                                .getElementById("ghana-card-back-upload")
                                .click()
                            }
                          >
                            <div className="flex flex-col items-center gap-3">
                            ...
                            <input
                              ...
                              accept="image/*"
                              onChange={(e) =>
                                handleGhanaCardBackUpload(e.target.files[0])
                              }
                              className="hidden"
                            />
                          </div>
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-700 mb-4">
                        Toggle account activation status. Inactive accounts
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-700">
                        Toggle verification status. Verified accounts have
      ...
      <AuthLayout>
        <div className="min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading landlord data...</p>
          </div>
        </div>
      </AuthLayout>
    );
